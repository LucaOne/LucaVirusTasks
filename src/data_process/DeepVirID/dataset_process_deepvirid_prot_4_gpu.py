...
@file: dataset_process_deepvirid_prot
@desc: xxxx
"""
import numpy as np
import random
...
dev_set = []
test_set = []
dev_negative_num = int(len(data[0]) * dev_rate)
dev_positive_num = int(len(data[1]) * dev_rate)
train_negative_num = len(data[0]) - 2 * dev_negative_num
train_positive_num = len(data[1]) - 2 * dev_positive_num
n_gpu = 4
# 保持训练集是4卡的倍数，不然4卡运行会报错
retain_train_negative_num = ((train_negative_num + train_positive_num)//n_gpu) * n_gpu - train_positive_num
for label in [0, 1]:
    ...
    if label == 0:
        real_train_negative_num = len(train)
        deleted_train_negative_num = real_train_negative_num - retain_train_negative_num
    else:
        deleted_train_negative_num = 0
    train_set.extend(train[deleted_train_negative_num:])
    if deleted_train_negative_num > 0:
        print("deleted: %d" % deleted_train_negative_num)
        csv_writer(
            dataset=train[: deleted_train_negative_num],
            handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/deleted_train_set_records.csv",
...
for dataset_type in ["train", "dev", "test"]:
    if not os.path.exists("../../../dataset/DeepVirID_4_gpu/protein/binary_class/%s" % dataset_type):
        os.makedirs("../../../dataset/DeepVirID_4_gpu/protein/binary_class/%s" % dataset_type)
csv_writer(
    dataset=train_set,
    handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/train/train.csv",
    header=["seq_id", "seq_type", "seq", "label"]
)

csv_writer(
    dataset=dev_set,
    handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/dev/dev.csv",
    header=["seq_id", "seq_type", "seq", "label"]
)

csv_writer(
    dataset=test_set,
    handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/test/test.csv",
    header=["seq_id", "seq_type", "seq", "label"]
)

csv_writer(
    dataset=[[0], [1]],
    handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/label.txt",
    header=["label"]
)

write_fasta("../../../dataset/DeepVirID_4_gpu/protein/binary_class/DeepVirID_4_gpu_all_prot.fasta", fasta)
...
if len(independent_test_fasta) > 0:
    ...
    if not os.path.exists("../../../dataset/DeepVirID_4_gpu/protein/binary_class/independent_test"):
        os.makedirs("../../../dataset/DeepVirID_4_gpu/protein/binary_class/independent_test")
    csv_writer(
        dataset=test_set,
        handle="../../../dataset/DeepVirID_4_gpu/protein/binary_class/independent_test/independent_test.csv",
        header=["seq_id", "seq_type", "seq", "label"]
    )
    write_fasta("../../../dataset/DeepVirID_4_gpu/protein/binary_class/independent_test_prot.fasta", independent_test_fasta)
...
seq len stats:
min: 25, max: 2979, mean: 406.104257, median: 301, 25: 245, 45: 301, 60: 310, 75: 437, 80: 503, 85: 622, 90: 795, 95: 1046, 99: 1940
Negative
deleted: 3
total: 3929010, train: 3143208, dev: 392901, test: 392901
Positive
total: 130967, train: 104775, dev: 13096, test: 13096
total: 4059974, train: 3247980, dev: 405997, test: 405997
process filename: ../../../data/DeepVirID_full/gene_protein/binary_class/test/test.csv
independent_seq_ids: 2000, independent_seqs: 2000
total: 2000, positive: 1000, negative: 1000, rate: 1000.000000
seq len stats:
min: 85, max: 8376, mean: 1076.457500, median: 625, 25: 354, 45: 553, 60: 825, 75: 1707, 80: 2068, 85: 2189, 90: 2402, 95:3050, 99: 4401
"""