...
try:
    ...
    from llm.lucavirus.models.lucaone_gplm import LucaGPLM
    from llm.lucavirus.models.lucaone_gplm_config import LucaGPLMConfig
    from llm.lucavirus.models.alphabet import Alphabet
...
def load_model(
    ...
    elif args_info["model_type"] in ["lucaone_virus", "lucavirus"]:
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        choices=["gene", "prot", "multi_gene", "multi_prot", "dna", "rna"],
...
def main(model_args):
    ...
    if seq_type not in ["gene", "prot", "multi_gene", "multi_prot", "dna", "rna"]:
        print("Error! arg: --seq_type=%s is not gene(dna or rna) or prot or multi_gene or multi_prot" % seq_type)
        sys.exit(-1)
