...
python -W ignore -m torch.distributed.launch --nnodes 1 --node_rank 0 --master_port=25687 --nproc_per_node=2 \
  ...
  --vector_dirpath ../../vectors/$DATASET_NAME/$DATASET_TYPE/$TASK_TYPE/$llm_type/$llm_version/$llm_time_str/$llm_step \
  --matrix_dirpath ../../matrices/$DATASET_NAME/$DATASET_TYPE/$TASK_TYPE/$llm_type/$llm_version/$llm_time_str/$llm_step \
  --seq_fc_size null \
