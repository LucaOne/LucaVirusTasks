...
try:
    ...
    from src.lucapair.models.LucaPairDecoderAB import LucaPairDecoderAB
    from src.lucapair.models.LucaPairDecoderDual import LucaPairDecoderDual
...
def get_args():
    ...
    parser.add_argument('--layer_norm_type', default="post", type=str, choices=["pre", "post"],
                        help='the layer norm type.')
    parser.add_argument('--layer_norm_name', default="LayerNorm", type=str, choices=["LayerNorm", "RMSNorm", "FlashRMSNorm"],
                        help='the layer norm name.')
    # 早停
...
def check_args(args):
    ...
    print("embedding_input_size_a", args.embedding_input_size_a)
    print("embedding_input_size_b", args.embedding_input_size_b)
    print("embedding_input_size_c", args.embedding_input_size_c)
...
def create_logger(args):
    ...
    if args.local_rank in [-1, 0]:
        ...
        if "#" not in args.vector_dirpath and not os.path.exists(args.vector_dirpath):
            os.makedirs(args.vector_dirpath)
        if "#" not in args.matrix_dirpath and not os.path.exists(args.matrix_dirpath):
...
def get_model(args):
    ...
    if args.layer_norm_type is not None:
        model_config.layer_norm_type = args.layer_norm_type
    if args.layer_norm_name is not None:
    ...
    if args.model_type == "lucaprot":
        ...
        model_class = LucaPairDecoderAB
    elif args.model_type in ["lucapair_decoder_dual"]:
        model_class = LucaPairDecoderDual
    ...
    if args.model_type in ["lucapair_decoder_ab", "lucapair_decoder_dual"]:
    ...
    elif args.model_type in ["lucapair_encoder_ab", "lucapair_encoder_dual"]:
...
def main():
    ...
    if "luca_separated" in args.llm_type:
    ...
    elif "lucavirus-gene" in args.llm_type and "lucavirus-prot" in args.llm_type:
        llm_version_strs = args.llm_version.split("-")
        if len(llm_version_strs) == 1:
            llm_version_strs = [llm_version_strs[0], llm_version_strs[0]]
        llm_time_str_strs = args.llm_time_str.split("-")
        if len(llm_time_str_strs) == 1:
            llm_time_str_strs = [llm_time_str_strs[0], llm_time_str_strs[0]]
        llm_step_strs = args.llm_step.split("-")
        if len(llm_step_strs) == 1:
            llm_step_strs = [llm_step_strs[0], llm_step_strs[0]]
        gene_str_idx = args.llm_type.index("gene")
        prot_str_idx = args.llm_type.index("prot")
        if gene_str_idx < prot_str_idx:
            llm_version_strs = llm_version_strs[::-1]
            llm_time_str_strs = llm_time_str_strs[::-1]
            llm_step_strs = llm_step_strs[::-1]
        # dataset_types = args.dataset_type.split("_")
        llm_dirpath_1 = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-%s" % (
            args.llm_dir if args.llm_dir else "..",
            llm_version_strs[0],
            args.llm_task_level,
            "lucavairus-gene",
        ...
        if not os.path.exists(llm_dirpath_1):
            llm_dirpath_1 = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-step%s" % (
                args.llm_dir if args.llm_dir else "..",
                llm_version_strs[0],
                args.llm_task_level,
                "lucavirus-gene",
        ...
        if len(llm_version_strs) > 1:
            llm_dirpath_2 = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-%s" % (
                args.llm_dir if args.llm_dir else "..",
                llm_version_strs[1],
                args.llm_task_level,
                "lucavirus-prot",
            ...
            if not os.path.exists(llm_dirpath_2):
                llm_dirpath_2 = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-step%s" % (
                    args.llm_dir if args.llm_dir else "..",
                    llm_version_strs[1],
                    args.llm_task_level,
                    "lucavirus-prot",
            ...
            llm_dirpath = {
                "gene": llm_dirpath_1,
                "protein": llm_dirpath_2
            }
        else:
            llm_dirpath = {
                args.dataset_type: llm_dirpath_1
            }
