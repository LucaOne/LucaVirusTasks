...
try:
    ...
    from llm.esm2.predict_embedding import predict_embedding as predict_embedding_esm2
    from llm.esmc.predict_embedding import predict_embedding as predict_embedding_esmc
    ...
    from src.llm.esm2.predict_embedding import predict_embedding as predict_embedding_esm2
    from src.llm.esmc.predict_embedding import predict_embedding as predict_embedding_esmc
...
class Encoder(object):
    def __init__(
        ...
        if "llm_version" in kwargs and kwargs["llm_version"]:
            self.llm_version = kwargs["llm_version"]
        if "llm_step" in kwargs and kwargs["llm_step"]:
            self.llm_step = kwargs["llm_step"]
        if "llm_time_str" in kwargs and kwargs["llm_time_str"]:
            self.llm_time_str = kwargs["llm_time_str"]
    ...
    def __get_embedding__(self, seq_id, seq_type, seq, embedding_type):
        ...
        if embedding_info is None:
            ...
            if "onehot" in self.llm_type:
            ...
            elif "esm" in self.llm_type and "prot" in seq_type and "esmc" in self.llm_version:
                if seq_type == "multi_prot":
                    ...
                    for cur_seq in cur_seqs:
                        ...
                        truncation_seq_length = self.seq_max_length - int(self.prepend_bos) - int(self.append_eos)
                        while True:
                            cur_embedding_info, cur_processed_seq_len = predict_embedding_esmc(
                                [seq_id, cur_seq],
                                self.trunc_type,
                                embedding_type,
                                repr_layers=[-1],
                    ...
                    cur_seq_len = len(seq)
                    while True:
                        embedding_info, processed_seq_len = predict_embedding_esmc(
                            [seq_id, seq],
                            self.trunc_type,
                            embedding_type,
                            repr_layers=[-1],
                ...
                if seq_type == "multi_prot":
                    ...
                    for cur_seq in cur_seqs:
                        ...
                        truncation_seq_length = self.seq_max_length - int(self.prepend_bos) - int(self.append_eos)
                        while True:
                            cur_embedding_info, cur_processed_seq_len = predict_embedding_esm2(
                                [seq_id, cur_seq],
                                self.trunc_type,
                                embedding_type,
                                repr_layers=[-1],
                    ...
                    cur_seq_len = len(seq)
                    while True:
                        embedding_info, processed_seq_len = predict_embedding_esm2(
                            [seq_id, seq],
                            self.trunc_type,
                            embedding_type,
                            repr_layers=[-1],
    ...
    def encode_triple(
        ...
        if self.input_type in ["matrix", "seq_matrix"] and "esm" in self.llm_type:
            ...
            if seq_type_c == "prot":
                seq_c = clean_seq_esm(seq_id_c, seq_c)
            elif seq_type_c == "multi_prot":
                seq_c = ",".join([clean_seq_esm(seq_id_c, v) for v in seq_c.split(",")])
